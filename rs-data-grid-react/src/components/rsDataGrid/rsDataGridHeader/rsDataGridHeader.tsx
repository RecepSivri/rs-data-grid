...
export const RsDataGridHeader = (param: IRsDataGridHeaderProps)  => {
  ...
  if(setHeight){
    ...
    heightOfAllHeader = (columnItem ? columnItem.customizeHeader ? columnItem.customizeHeader.height : '20px' : '20px');
        ...
        style={{
        ...
        }}>
          {
            ...
            Object.keys(data[0]).map((item: any, index: number) => {
              ...
              style={{
                borderRight: index !== Object.keys(data[0]).length -1 ?   (border.borderColor ? '1px solid '+ border.borderColor : '1px solid #ccc') : "",
              }}>{item}</div>
            })
          }
          {
            ...
            column.map((item: IColumn, index: number) => {
              return <div className='rs-data-grid-header-item row-layout-center' key={'rs-data-grid-header-item-'+index}
              style={{
                borderRight: border ? border.borderInnerHorizontal ? (index !== column.length -1 ?   (border.borderColor ? '1px solid '+ border.borderColor : '1px solid #ccc') : "") : "" : "",
                width: item.customize ? (item.customize.width ? item.customize.width : 'inherit') : '100%',
                height: heightOfAllHeader
              }}>{item.customizeHeader ? item.customizeHeader.template ? item.customizeHeader.template: item.name: item.name }</div>
            })
          }
        </div>
    </div>
