...
import { IColumn, ICustomization, IRsDataGridData } from "./models/rsDataGrid.models";
...
export const RsDataGrid = (param: IRsDataGridProps)  => {
  ...
  const customizationInit: ICustomization = {
    ...
    page: {
      page: customization ? customization.page ? customization.page.page ? customization.page.page : 0 : 0 : 0,
      pageCurrSize: customization ? customization.page ? customization.page.pageCurrSize ? customization.page.pageCurrSize : 5 : 5 : 5,
      pageSizeList:  customization ? customization.page ? customization.page.pageSizeList ? customization.page.pageSizeList : [10, 20, 40, 50, 100] : [10, 20, 40, 50, 100]  : [10, 20, 40, 50, 100],
      pageSize: customization ? customization.page ? customization.page.pageSize ? customization.page.pageSize : 10 : 10 : 10,
    }
  }
  ...
  const dataTableInitialState: IRsDataGridData = {
    data: data ? data : [],
    page: {
      page: customizationInit ? customizationInit.page ? customizationInit.page.page : 0 : 0,
      pageCurrSize: customizationInit ? customizationInit.page ? customizationInit.page.pageCurrSize : 5 : 5,
      pageSizeList: customizationInit ? customizationInit.page ? customizationInit.page.pageSizeList : [10, 20, 40, 50, 100] : [10, 20, 40, 50, 100],
      pageSize: customizationInit ? customizationInit.page ? customizationInit.page.pageSize :10 : 10,
      length: customizationInit ? customizationInit.page ? Math.ceil((data ? data : []).length / customizationInit.page.pageSize) : 0 : 0
    }
  }
  ...
  useEffect(() => {
    if(fetchData){
      fetch(fetchData ? fetchData : '').then( (response: any) => {
        ...
        style={{
        ...
        }}>
          {
            ...
            <TableStateContext.Provider value={{dataTableState, setDataTable}}>
              <RsDataGridHeader column= {column} customization = {customizationInit}/>
            </TableStateContext.Provider>
          }
          {
            dataTableState.data &&
            <TableStateContext.Provider value={{dataTableState, setDataTable}}>
              <RsDataGridTable  column= {column} customization = {customizationInit}/>
            </TableStateContext.Provider>
          }
        </div>
        {
