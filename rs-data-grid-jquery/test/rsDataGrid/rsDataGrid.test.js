...
vi.mock('../../src/rsDataGrid/dialogs/confirmDialog.js', () => ({ requestConfirm: (...args) => requestConfirm(...args) }));
vi.mock('../../src/rsDataGrid/dialogs/editRowDialog.js', () => ({ requestEditRow: (...args) => requestEditRow(...args) }));
vi.mock('../../src/rsDataGrid/dialogs/gridSettingsDialog.js', () => ({ openGridSettings: (...args) => openGridSettings(...args) }));
vi.mock('../../src/rsDataGrid/exporters.js', () => ({ exportExcel: (...args) => exportExcel(...args), exportPdf: (...args) => exportPdf(...args) }));
...
describe('columns: inference, drag-reorder persistence, Grid Settings filtering', () => {
  ...
  it('reorders columns via drag-and-drop (onColumnMove) and keeps the order across a data update', () => {
    ...
    dragHandles[0].dispatchEvent(new Event('dragstart', { bubbles: true }));
  ...
  it('drops fields from the retained drag order once they no longer exist, and appends genuinely new fields at the end', () => {
    ...
    dragHandles()[0].dispatchEvent(new Event('dragstart', { bubbles: true }));
    Array.from(container.querySelectorAll('.content-style'))[1].dispatchEvent(new Event('drop', { bubbles: true }));
  ...
  it('dropping a dragged column onto itself is a no-op', () => {
    ...
    container.querySelectorAll('.content-style')[0].dispatchEvent(new Event('drop', { bubbles: true }));
  ...
  it('dragging a column backward (later index onto an earlier one) reorders correctly', () => {
    ...
    handles[2].dispatchEvent(new Event('dragstart', { bubbles: true })); // drag "Genre"...
    container.querySelectorAll('.content-style')[0].dispatchEvent(new Event('drop', { bubbles: true })); // ...onto "Title"
    const captions = Array.from(container.querySelectorAll('.header-caption')).map(el => el.textContent);
...
describe('row drag-drop -> store.moveRow', () => {
  it('reordering rows via drag calls the store, reflected in render order', () => {
    ...
    handle.dispatchEvent(new Event('dragstart', { bubbles: true }));
    dataRows()[2].dispatchEvent(new Event('drop', { bubbles: true }));
